/*
  # Create educational levels and academic groups tables
...
-- Insert educational levels with proper UUID format
DO $$
DECLARE
  level0_id uuid := gen_random_uuid();
  level1_id uuid := gen_random_uuid();
  level2_id uuid := gen_random_uuid();
  group0_id uuid := gen_random_uuid();
  group1_id uuid := gen_random_uuid();
  group2_id uuid := gen_random_uuid();
  group_archived_id uuid := gen_random_uuid();
BEGIN
  -- Insert educational levels
  INSERT INTO educational_levels (id, name, description, order_num, is_active) VALUES
    (level0_id, 'Educación Infantil', 'Niveles iniciales de educación infantil (3-6 años)', 0, true),
    (level1_id, 'Educación Primaria', 'Niveles básicos de educación primaria', 1, true),
    (level2_id, 'Educación Secundaria', 'Niveles intermedios de educación secundaria', 2, true);
